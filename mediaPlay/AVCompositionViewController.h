...
/*
 ...
 @class          AVComposition
 @abstract       An AVComposition combines media data from multiple（多样的） local file-based sources in a custom temporal（暂时，临时） arrangement（布置，排列）, in order to present or process（处理。加工） media data from multiple sources together. All local file-based audiovisual assets are eligible（适合、合适） to be combined, regardless（不用管） of container type.
 
 @discussion
 At its top-level, AVComposition is a collection of tracks, each presenting media of a specific media type, e.g. audio or video, according to a timeline. Each track is represented（代表） by an instance of AVCompositionTrack.
 
 Each（每一个） track is comprised（包含） of an array of track segments, each of which present a portion of（部分） the media data stored in a source container, specified by URL, a track identifier, and a time mapping, as represented（代表） by an instance of AVCompositionTrackSegment.
